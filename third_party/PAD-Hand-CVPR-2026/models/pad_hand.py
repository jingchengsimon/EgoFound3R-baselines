...
class MeshConv(nn.Module):
    def __init__(self, device, latent_channels=128, in_channels=3):
        ...
        template_fp = "./assets/MeshConv_template.ply"
        transform_fp = "./assets/MeshConv_transform.pkl"
        ds_factors = [2, 2, 2, 2]
        seq_length = [9, 9, 9, 9]
        dilation = [1, 1, 1, 1]
...
class PAD_Hand(nn.Module):
    ...
    def forward(
        ...
        x0_pose_encoder_pred = self.pose_decoder(z_embedding) if copied_pose_decoder is None else copied_pose_decoder(z_embedding)
        ...
        x0_mano_encoder_pred_t = mano.forward(
            betas=x0_shape_encoder_pred_t if shapes is None else shapes.reshape(-1, 10), thetas_rotmat=x0_rotmat_encoder_pred_t
        )
        ...
        if self.decoder:
            ...
            if copied_pose_decoder is not None:
                x0_mano_decoder_pred_t = None
                x0_rotmat_decoder_pred_t = None
            else:
                x0_rotmat_decoder_pred_t = rot6d_to_rotmat(
                    x0_pose_decoder_pred.reshape([-1, 6])
                ).view(-1, 16, 3, 3)
                x0_mano_decoder_pred_t = mano.forward(
                    betas=x0_shape_decoder_pred_t if shapes is None else shapes.reshape(-1, 10), thetas_rotmat=x0_rotmat_decoder_pred_t
                )
        else:
            ...
            x0_rotmat_decoder_pred_t = x0_rotmat_encoder_pred_t
            x0_shape_decoder_pred_t = x0_shape_encoder_pred_t
            x0_mano_decoder_pred_t = x0_mano_encoder_pred_t

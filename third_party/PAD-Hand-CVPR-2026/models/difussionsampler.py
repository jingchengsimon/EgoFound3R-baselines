...
def get_named_beta_schedule(num_diffusion_steps, p=0.1):
    ...
    """
    return [(n-1)**p / (num_diffusion_steps-1)**(p-1) for n in range(1, num_diffusion_steps+1)]
...
class DiffusionSampler(object):
    ...
    def __init__(
        ...
        if num_diffusion_steps == 1:
            ...
            self.sqrt_etas_schedule = get_named_eta_schedule(num_diffusion_steps, kappa, self.beta_schedule)
        
        self.alpha_schedule = self.sqrt_etas_schedule - np.r_[0, self.sqrt_etas_schedule[:-1]]
        print('sqrt_etas:')
        print(self.sqrt_etas_schedule)
        self.sqrt_alphas_schedule = self.sqrt_etas_schedule - np.r_[0, self.sqrt_etas_schedule[:-1]]
        self.sqrt_etas = torch.tensor(self.sqrt_etas_schedule)
        self.sqrt_alphas = torch.tensor(self.sqrt_alphas_schedule)
        self.device = device
    ...
    def q_sample(self, x_0, e, n, mean=False):
        ...
        """
        index_value = torch.tensor([self.sqrt_etas[n_b-1] for n_b in n]).unsqueeze(1).unsqueeze(2).float().to(self.device)
        
        return index_value**2 * e + x_0 + index_value * self.kappa * torch.randn_like(x_0).to(self.device)

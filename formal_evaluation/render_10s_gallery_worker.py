    ...
    try:
        for i, win in enumerate(row["windows"]):
            ...
            for method in ("wilor", "hawor", "pad_hand", "reviv4d"):
                ...
                if method == "pad_hand" and not has_pad_mesh(directory / "predictions.npz"):
                    mesh_roots = spec[ds].get("pad_hand_mesh_roots", [])
                    if not mesh_roots:
                        raise ValueError(f"PAD_HAND_VERTEX778_SOURCE_NOT_REGISTERED:{cache_id}")
                    mesh_directory = one_prediction(mesh_roots, cache_id, method, ds, ids)
                    if not has_pad_mesh(mesh_directory / "predictions.npz"):
                        raise ValueError(f"PAD_HAND_VERTEX778_SOURCE_INVALID:{mesh_directory}")
                    link(mesh_directory / "predictions.npz", staging / f"{i}_pad_hand_mesh.npz", evidence)
                    link(mesh_directory / "metadata.json", staging / f"{i}_pad_hand_mesh_metadata.json", evidence)
            for t in (0, 149, 299):
                if t // 60 == i:
...
def main():
    ...
    parser.add_argument("--workspace", type=Path,
                        help="unique input workspace for a new gallery version")
    parser.add_argument("--source-alignment", type=Path,
                        help="validated source alignment including registered PAD-Hand mesh roots")
    ...
    spec_path = args.source_alignment or args.runtime / "visualization/batch_10s_177_p95_wmpjpe_20260912/source_alignment_5000.json"
    ...
    workspace = args.workspace or args.runtime / "visualization/batch_10s_177_p95_wmpjpe_20260912"
    ...
    for row in rows:
        ...
        try:
            prepare_inputs(row, spec, workspace, indices[ds])
            renderer.main(row["segment_id"], args.output, workspace)
            completed += 1
